...
const showOrderInfo = (data) => {
  ...
  modalBody.insertAdjacentHTML(
    'beforeend',
    `
    <h2 class="modal__title">Замовлення оформлено №${data.id}</h2>
    <p class="modal__description modal__description_thank">Дякуємо за вибір нашого магазину!</p>
    <p class="modal__description">Тут ви можете переглянути всі деталі замовлення.</p>

    <ul class="modal__customer-data customer">
      <li class="customer__item">
        <span class="customer__item-title">Отримувач</span>
        <span class="customer__item-data">${data.fio}</span>
      </li>

      ${data.address && `
        <li class="customer__item">
          <span class="customer__item-title">Адреса доставки</span>
      ...
      <li class="customer__item">
        <span class="customer__item-title">Спосіб отримання</span>
  ...
  const cartTotal = createElement(
    'div',
    {
      ...
      innerHTML: '<p class="modal__total-title">Разом:</p>'
    },
    {
      parent: modalBody,
    }
...
export const renderOrder = ({render}) => {
  ...
  const container = createElement(
    ...
    {
      className: 'container',
      innerHTML: '<h2 class="order__title">Оформлення замовлення</h2>'
    },
    {
      parent: order
    }
  ...
  orderForm.insertAdjacentHTML('beforeend', `
    <fieldset class="order__personal">
      <label class="order__label">
        <input class="order__input" type="text" placeholder="ФІО" name="fio" required>
      </label>            
      
      <label class="order__label">
        <input class="order__input" type="text" placeholder="Адреса доставки" name="address">
      </label>            
    ...
    <button class="order__submit main-button" type="submit">Оформити</button>
